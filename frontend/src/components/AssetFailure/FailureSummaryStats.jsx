...
const FailureSummaryStats = ({ assets }) => {
  const calculateSummaryStats = () => {
    const totalAssets = assets.length;

    // Use the status field directly from your API
    const healthyAssets = assets.filter(a => a.status === "Normal").length;
    const criticalAssets = assets.filter(a => a.status === "High Risk").length;
    const warningAssets = 0; // No warning status in your data
  ...
  const statsData = [
    {
      ...
      progressWidth: `${Math.min(stats.healthPercentage, 100)}%`
    },
    {
      ...
      progressWidth: `${Math.min((stats.criticalAssets / stats.totalAssets) * 100, 100)}%`
    },
    {
      icon: "fas fa-bell",
      title: "هشدارها",
      ...
      progressWidth: `${Math.min((stats.warningAssets / stats.totalAssets) * 100, 100)}%`
    },
    {
...
export default FailureSummaryStats;