...
export const getPredictionUpdates = async () => {
  const res = await fetch(`${API_URL}/pdm/prediction-updates/`);
  if (!res.ok) throw new Error("خطا در دریافت داده‌های پیش‌بینی");
  return await res.json();
};

export const getPredictionByType = async (type) => {
  const res = await fetch(`${API_URL}/pdm/prediction-updates/get_by_type/?type=${type}`);
...
export const updatePredictionStatus = async (id, status, progress = 0, errorMessage = null) => {
  const res = await fetch(`${API_URL}/pdm/prediction-updates/${id}/update_status/`, {
    method: "POST",
...
export const setPredictionSchedule = async (id, scheduleConfig) => {
  const res = await fetch(`${API_URL}/pdm/prediction-updates/${id}/set_schedule/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const clearPredictionSchedule = async (id) => {
  const res = await fetch(`${API_URL}/pdm/prediction-updates/${id}/clear_schedule/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
...
export const startLifetimePrediction = async () => {
  const res = await fetch(`${API_URL}/rul-predictions/predict_all_assets/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      asset: null,
...
export const generateMonitoringReport = async (modelType) => {
  const res = await fetch(`${API_URL}/pdm/monitoring/generate-report/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model_type: modelType }),
  });
  if (!res.ok) throw new Error("خطا در تولید گزارش مانیتورینگ");
  return await res.json();
};

export const getReportStatus = async (modelType) => {
  const res = await fetch(`${API_URL}/pdm/monitoring/report-status/?model_type=${modelType}`);
  if (!res.ok) throw new Error("خطا در دریافت وضعیت گزارش");
  return await res.json();
};

export const downloadReport = async (modelType) => {
  const res = await fetch(`${API_URL}/pdm/monitoring/download-report/?model_type=${modelType}`);
